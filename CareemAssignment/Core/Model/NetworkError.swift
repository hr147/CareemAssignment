...
public enum NetworkError: Error, CustomStringConvertible {
    ...
    init(graphQLErrors:[String]?) {
        
        if let graphErrors = graphQLErrors{
            
            let errors = Array(Set(graphErrors)).joined(separator: "\n")
            
            self = .ServerError(message: errors)
            
        }else {
        
            self = .ServerError(message: "Something went wrong on server. please try again.")
        }
